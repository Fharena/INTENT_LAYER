...
| --- | ---: | ---: |
| `src/App.tsx` | 13 | avg 1.771ms / p95 5.801ms / max 5.801ms |
...
| Overall average transform time | 0.889ms |
| Overall p95 transform time | 5.801ms |
| Overall max transform time | 5.801ms |
| Warm average transform time | 0.384ms |
| Warm p95 transform time | 0.861ms |
| Warm max transform time | 0.861ms |
| Warm target | <= 5ms |
| Cold target | <= 10ms |
...
| Iterations | 5 |
| Average transform time | 11.92ms |
| p95 transform time | 16.367ms |
| Max transform time | 16.367ms |
| Stress target | <= 20ms |
| Result | pass |
...
| Graph file | `.intent/tmp/vite-product-multi-file-graph-refresh/.intent/graph.intent.json` |
| File count | 24 |
| Cards per file | 6 |
| Initial graph entries | 624 |
...
| Unchanged sample files | 5 |
| Unchanged file tokens retained | true |
| Same-input generatedAt stable | true |
| Changed-input generatedAt update | true |
| Post-change repeat generatedAt stable | true |
| Entry count stable | true |
| Initial full transform time | 487.552ms |
| Same-input full repeat time | 559.311ms |
| Changed-file transform time | 20.498ms |
| Changed-file repeat transform time | 9.496ms |
| Changed-file target | <= 50ms |
| Result | pass |

Interpretation:

- The fixture uses 24 TSX files with 624 total graph entries, then changes only one file from `gap-4` to `gap-8`.
...
| --- | ---: | ---: | --- | ---: | ---: | ---: | ---: | --- |
| `shadcn-ui-ui-dbf9c5e` | 24 | 102 | `flex -> grid` | 110.263ms | 78.228ms | 5.701ms | 2.605ms | pass |
| `sadmann7/skateshop@e954d54` | 24 | 104 | `flex -> grid` | 61.686ms | 53.754ms | 4.015ms | 1.303ms | pass |
| `mckaywrigley/chatbot-ui@81328b6` | 24 | 135 | `w-full -> w-1/2` | 116.016ms | 87.772ms | 5.79ms | 2.801ms | pass |
...
| Preview time | 0.858ms |
| Preview round trip | 1.174ms |
| Apply success | true |
| Static apply time | 25.963ms |
| Simple `cn()` apply time | 10.808ms |
| Revert success | true |
| Revert time | 6.036ms |
...
| Fail count | 1 |
| Guidance count | 3 |
| Includes `vite-plugin` fail | true |
| Includes source-files pass | true |
| Includes `intent-layer/vite` guidance | true |
| Missing-plugin doctor time | 2.436ms |
...
- `doctor` self-checks package/Vite/React/Tailwind/source/plugin/intent graph state across 10 checks; all 10 pass in this fixture.
- The missing-plugin fixture rejects a Vite config without `intentLayer()` with a non-zero exit code and guidance to register `intent-layer/vite`.
...
| Installed transform hook time | 7.798ms |
...
| Installed Vite dev server setup status | 200 |
| Installed Vite dev server setup apply status | 200 |
| Installed Vite dev server setup language | `ko` |
| Installed Vite dev server setup workspace ready | true |
| Installed Vite dev server setup settings ready | true |
| Installed Vite dev server setup graph ready | true |
| Installed Vite dev server setup settings file exists | true |
| Installed Vite dev server setup schema exists | true |
| Installed Vite dev server settings update status | 200 |
| Installed Vite dev server settings language | `en` |
| Installed Vite dev server settings dock | `left` |
| Installed Vite dev server settings density | `compact` |
| Installed Vite dev server settings default collapsed | true |
| Installed Vite dev server settings auto-open setup | false |
| Installed Vite dev server settings Codex command | `codex-custom` |
| Installed Vite dev server settings Claude command | `claude-custom` |
| Installed Vite dev server settings command source | `settings` |
| Installed Vite dev server settings update ok | true |
...
| Installed Vite dev server apply refresh time | 262.18ms |
| Installed Vite dev server apply refresh target | <= 2500ms |
...
| Installed Vite dev server revert refresh time | 1747.146ms |
| Installed Vite dev server revert refresh target | <= 2500ms |
...
| Installed Vite dev server multi-file refresh time | 13.724ms |
| Installed Vite dev server multi-file refresh target | <= 2500ms |
| Installed Vite dev server multi-file refresh result | pass |
| Installed Vite dev server multi-file total time | 57.242ms |
| Installed Vite dev server smoke time | 5816.871ms |
| Dry-run time | 2990.229ms |
| Pack time | 3505.776ms |
| Install time | 4856.004ms |
| Installed help time | 3274.633ms |
| `/vite` import time | 1757.463ms |
| Installed transform smoke time | 1427.52ms |
...
- In the same temp install folder, it also starts a real Vite dev server and fetches/calls `/`, `/src/App.tsx`, `/__intent/graph`, `/__intent/setup`, `/__intent/preview`, `/__intent/apply`, and `/__intent/revert-last` over HTTP to verify the module transform, server middleware, first-run setup, safe patch apply, and revert together.
- The setup smoke verifies status 200 for setup read/apply, Korean language persistence, `.intent/settings.json`, schema output, workspace readiness, settings readiness, and graph readiness.
- The same setup smoke then changes settings mid-session to English, left dock, compact density, start-minimized true, setup auto-open false, custom Codex/Claude command strings, and verifies the command source is `settings`.
- The installed Vite dev server smoke verifies that a `gap-4 -> gap-6` patch reaches source, writes operation/diff/log artifacts, exposes one pending undo entry, and updates `/src/App.tsx` plus `/__intent/graph` within 262.18ms after apply.
- The same smoke then calls `/__intent/revert-last`, verifies source/module/graph return to `gap-4`, clears pending undo history, and refreshes within 1747.146ms.
- In the same Vite dev server session, it also loads App/Header/Card as three TSX graph files, changes only Card from `gap-4 -> gap-8`, then verifies the graph keeps three entries, updates the changed-file token, retains unchanged files, changes graph `generatedAt`, and refreshes the module/graph in 13.724ms.
- The verified package export is currently `intent-layer/vite`. External-facing onboarding/install/failure guide copy is included in the package tarball as `ONBOARDING_KR/EN.md`, `INSTALL_KR/EN.md`, and `FAILURE_MODES_KR/EN.md`.
...
| package install smoke | pack dry-run + tarball install + installed `intent-layer --help` includes doctor + installed `/vite` import + installed plugin transform/graph + installed Vite dev server HTTP graph/setup/preview/apply/revert + first-run setup + mid-session settings update + apply/revert refresh <= 2500ms + 3-file graph refresh <= 2500ms + context-pack excluded | pass |
...
| CLI agent launch | Codex/Claude command plans created + default executed=false + task creation/reuse paths verified | pass |
...
- 24-file/624-binding product-sized multi-file graph refresh gate pass
- package/plugin name aligned to `intent-layer` with `intent-layer/vite` install smoke pass
...
- package tarball dry-run, real pack, temp install, installed `intent-layer --help`, installed `intent-layer/vite` import, installed plugin transform/graph, and installed Vite dev server HTTP graph/setup/preview/apply/revert source patch plus first-run setup persistence, mid-session Settings persistence, 262.18ms apply refresh, 1747.146ms revert refresh, and 13.724ms 3-file graph refresh gate pass
...
- CLI tarball install, package `intent-layer/vite` wrapper export smoke, installed plugin transform/graph smoke, installed Vite dev server HTTP setup/settings/preview/apply/revert, setup settings/schema persistence, apply/revert refresh, 3-file graph refresh smoke, and onboarding/install/failure guide tarball inclusion pass, but npm registry publish and registry-oriented install copy remain
