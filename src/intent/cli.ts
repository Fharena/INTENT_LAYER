...
import { initIntentWorkspace as initializeIntentWorkspace } from "./setup";
import { configureViteProject, type ViteProjectSetupResult } from "./viteSetup";
...
function summarizeFiles(files: ScanFileResult[], startupTransformMs = 0): ScanSummary {
...
function scan(rootDir: string, inputs: string[]) {
  ...
  const startupTransformMs = files[0]
    ? instrumentSource({
        code: fs.readFileSync(files[0], "utf8"),
        file: files[0],
        rootDir
      }).transformMs
    : 0;
  ...
  return {
    entries,
    files: results,
    summary: summarizeFiles(results, startupTransformMs)
  };
}
...
function initIntentWorkspace(rootDir: string): CliInitReport {
  const started = performance.now();
  const workspace = initializeIntentWorkspace(rootDir);
  const vite = configureViteProject(rootDir);
  return {
    version: 1,
    command: "init",
    generatedAt: new Date().toISOString(),
    ok: vite.ok,
    root: rootDir,
    createdPaths: workspace.createdPaths,
    existingPaths: workspace.existingPaths,
    vite,
...
function usage(): string {
  return [
    "Usage:",
    "  intent-layer init  # create workspace and configure Vite",
