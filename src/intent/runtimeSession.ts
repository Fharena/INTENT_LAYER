...
export function readRuntimeSelection(rootDir: string): IntentRuntimeSelection {
  const file = selectionPath(rootDir);
  if (!fs.existsSync(file)) {
    return { version: 1, selectedAt: new Date(0).toISOString(), selection: null };
  }
  try {
    const value = JSON.parse(fs.readFileSync(file, "utf8")) as IntentRuntimeSelection;
    return value.version === 1 && "selection" in value
      ? value
      : { version: 1, selectedAt: new Date(0).toISOString(), selection: null };
  } catch {
    return { version: 1, selectedAt: new Date(0).toISOString(), selection: null };
  }
}
