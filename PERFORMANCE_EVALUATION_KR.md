...
다음 우선순위는 준비된 external corpus harness로 독립 외부 샘플 50-100개를 실제 측정하고, 임의 깊이 transitive data-flow를 어디까지 MVP에서 막을지 정한 뒤 외부 제품급 TSX 파일에서 component snapshot 및 product-sized graph throttle 재측정을 진행하는 것이다.
```
