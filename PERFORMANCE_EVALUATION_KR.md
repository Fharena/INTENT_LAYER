...
- 현재 CLI MVP는 `scan`/`check`만 구현했고, `init`, `dev`, `diff`, `apply` 같은 명령은 아직 별도 구현하지 않았다.
...
- CLI는 현재 `scan`/`check`만 구현되어 있고, `init/dev/diff/apply` 명령은 출시 polish로 남아 있다.
