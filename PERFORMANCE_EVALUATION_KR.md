...
| max file transform | 0.3ms | <= 20ms | 통과 |
...
- `src/intent/cli.ts`는 별도 서버 없이 현재 instrumentation 엔진을 직접 호출한다.
...
- 현재 CLI MVP는 `init`/`scan`/`check`/`apply`/`diff`/`agent-context`/`agent-task`/`agent-result`를 구현했다. `dev` 래핑은 출시 polish로 남아 있다.
...
- CLI는 현재 `init`/`scan`/`check`/`apply`/`diff`/`agent-context`/`agent-task`/`agent-result`를 구현했지만, `dev` 래핑은 출시 polish로 남아 있다.
