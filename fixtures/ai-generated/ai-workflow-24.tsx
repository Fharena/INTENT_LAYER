...
// Source label: ai-workflow-24

declare function cn(...value: Array<string | false | undefined>): string;

export function AiWorkflow24({ active = false, locked = false }: { active?: boolean; locked?: boolean }) {
  ...
  return (
    <section className="flex flex-col gap-4 rounded-2xl border border-stone-200 bg-white p-6">
      <div className={cn("grid grid-cols-4 gap-3", locked && "opacity-60", statusClass)}>
        ...
        <button className="rounded-lg bg-rose-100 px-4 py-2 text-sm font-semibold text-rose-800">Audit</button>
      </div>
    </section>
  );
}
