...
MVP의 related source snapshot은 같은 파일 변수 선언, 같은 파일 one-hop 변수 dependency snapshot, workspace package import 뒤의 변수 선언, tsconfig paths alias와 다단계 barrel re-export 뒤의 imported 변수 선언, one-hop relative named import, tsconfig paths alias와 one-hop/multi-hop named barrel re-export 뒤의 `variant/cva` 선언까지 지원한다.
external npm package import, variant 함수 의미 분석, cross-file/transitive variable data flow는 agent handoff 문서에 명시된 미지원 범위로 남긴다.
...
2. docs
3. landing README
4. install guide: local tarball install, `/vite` wrapper export smoke, 설치된 plugin transform/graph smoke, 실제 Vite dev server HTTP preview/apply 및 3-file graph refresh smoke는 구현됨, public package 이름/문구는 남음
